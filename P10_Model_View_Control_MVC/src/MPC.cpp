...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    int weight_cte = 1; 
    int weight_epsi = 1; 
    int weight_actuator_delta = 1; 
    int weight_actuator_a = 1;
    int weight_delta_start = 1; 
    ...
    for (int i = 0; i < N - 1; i++) {
      fg[0] += CppAD::pow(vars[delta_start + i], 2) * weight_actuator_delta; 
      fg[0] += CppAD::pow(vars[a_start + i], 2) * weight_actuator_a; 
    }

    // // Minimize the value gap between sequential actuations.
    for (int i = 0; i < N - 2; i++) {
      fg[0] += CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2) * weight_delta_start; 
    ...
    for (int i = 0; i < N - 1; i++) {
    	...
    	// consider a third order polynomial to calculate constrains below // tutorial solution: linear equation; here third order polynomial!
