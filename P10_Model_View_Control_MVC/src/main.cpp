    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          // Acknowledgements:
          // https://github.com/udacity/CarND-MPC-Quizzes/tree/master/polyfit

          // get veh coordinate system points from reference points:
          ...
          // Fit Polynomial and compute cte and epsi error:
          auto coeffs = polyfit(ptsxn, ptsyn, 3);
          double cte = polyeval(coeffs, px) - py;
          double epsi = psi - atan(coeffs[1] + (2 * coeffs[2] * px) + (3 * coeffs[3]* (px*px)));

          // Copmute state vector:
